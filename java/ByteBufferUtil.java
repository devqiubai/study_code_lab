...
public final class ByteBufferUtil {
  // 16 Kb
  private static final int BUFFER_SIZE = 16384; //缓存大小 16*1024
  ...
  public static ByteBuffer fromFile(@NonNull File file) throws IOException {
    ...
    try {
      ...
      // See #2240.
      if (fileLength > Integer.MAX_VALUE) { //还有这种case 文件长度大于in.max
        throw new IOException("File too large to map into memory");
      }
      // See b/67710449.
      if (fileLength == 0) { //length == 0 判断
        throw new IOException("File unsuitable for memory mapping");
      }
    //randomAccessFile r -read模式
